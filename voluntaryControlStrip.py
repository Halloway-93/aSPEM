...
def run_trial(numberOfTrials, trial_index,p_up=0.9,p_down=0.1):
    ...
    button_pos = 150*(2*stats.bernoulli.rvs(.5)-1) # Horizontal offset from the center of the screen
    ...
    up_arrow = visual.ShapeStim(
        win=win,
        vertices=((0, arrow_height), (-arrow_width, -arrow_height), (arrow_width, -arrow_height)),
        lineWidth=3,
        lineColor='white',
        fillColor='white',
        pos=(button_pos, 0)
    ...
    if button_pos=='right':
        print("the up arrow is on the right")
    else:
        print("the up arrow is on the left")
    
    while (not dummy_mode) and (trial_index==50 or trial_index==100 or trial_index==150 or trial_index==200):
    ...
    chosen_arrow = None
    while chosen_arrow not in ['left', 'right']:
        keys = event.waitKeys(keyList=['left', 'right'])
        if keys:
            chosen_arrow = keys[0]

    # Map the keypresses to color choices
    if chosen_arrow == button_pos:
        chosen_arrow = 'up'
    else:
        chosen_arrow= 'down'
    
    print("the chosen arrow is:",chosen_arrow)
    ...
    upper_rdk = visual.DotStim(win, nDots=num_dots, dotSize=5, fieldSize=(band_width_px, band_height_px), dotLife=30,
                            color="white", coherence=0, dir=0, fieldPos=(0, int(win.size[1] / 8)),speed=1.5)
    ...
    #green stimuli
    # Probability of the band going right
    dir_up = stats.bernoulli.rvs(p_up)
    if dir_up == 1:
        dir_up = 0
    else:
        dir_up = 180
    
    upper_rdk.dir = dir_up

    #red stimuli
    # Probability of the band going right
    dir_down = stats.bernoulli.rvs(p_down)
    if dir_down == 1:
        dir_down = 0
    else:
        dir_down = 180

    lower_rdk.dir = dir_down
            ...
            if new_sample is not None:
                if old_sample is not None:
                    if new_sample.getTime() != old_sample.getTime():
                        ...
                        if chosen_arrow == 'up':
                            fix_y =  upper_rdk.pos[1]
                        else:
                            fix_y =  lower_rdk.pos[1]
...
task_msg = 'In the task, please select one direction UP or DOWN with the RIGHT or LEFT keyboard arrow\n' + \
    'then make a saccade toward the strip of the selected direction\n' + \
...
for i in range(numberOfTrials):
    run_trial(numberOfTrials, trial_index)
    trial_index += 1
