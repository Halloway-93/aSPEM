...
all_screens = pyglet.canvas.get_display().get_screens()
...
aspect_ratio = scn_width / scn_height

# Set the band height to occupy around 40% of the screen's height
band_height_px = int(scn_height * 0.4)

mon = monitors.Monitor('SecondaryMonitor', width=aspect_ratio, distance=70.0)
win = visual.Window(size=(scn_width, scn_height), color=(0.5, 0.5, 0.5), units='pix', monitor=mon, fullscr=True, screen=1)

# Get the Pyglet window object associated with the Psychopy window
pyglet_win = win.winHandle

# Set the position of the window on the secondary display
pyglet_win.set_location(secondary_screen.x, secondary_screen.y)
...
for trial in range(Ntrials):
    ...
    upper_rdk = visual.DotStim(win, nDots=num_dots, dotSize=5, fieldSize=(band_width_px, band_height_px), dotLife=30,
                               color='green', coherence=0, dir=0, fieldPos=(0, green_band_y))

    lower_rdk = visual.DotStim(win, nDots=num_dots, dotSize=5, fieldSize=(band_width_px, band_height_px), dotLife=30,
                               color='red', coherence=0, dir=0, fieldPos=(0, red_band_y))

    # Set the direction for the RDK stimuli based on the chosen color
    if chosen_color == 'g':
    ...
    timer = core.CountdownTimer(2.0)  # Set timer for 1000ms (2 seconds)
